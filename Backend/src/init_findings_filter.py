...
from settings import ENABLE_LLM_FILTERING
from .init_llm_client import LLMClient
...
class FindingsFilter:
    ...
    def __init__(self, 
        ...
        # Initialize api client if filtering is enabled
        self.client = None
    ...
    def filter_findings(self, 
        ...
        if self.use_claude_filtering and self.client and findings_after_hard:
            ...
            for orig_idx, finding in findings_after_hard:
                ...
                success, analysis_result, error_msg = self.client.analyze_single_finding(
            ...
            for orig_idx, finding in findings_after_hard:
                ...
                enriched_finding['_filter_metadata'] = {
                    'confidence_score': 10.0,  # Default high confidence
                    'justification': 'LLM filtering disabled',
                }
                findings_after_claude.append(enriched_finding)
                stats.kept_findings += 1
...
def initialize_findings_filter(custom_filtering_instructions: Optional[str] = None) -> FindingsFilter:
    ...
    try:
        ...
        use_claude_filtering = ENABLE_LLM_FILTERING
        ...
        raise (f'Failed to initialize findings filter: {str(e)}')
