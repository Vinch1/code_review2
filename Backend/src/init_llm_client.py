...
from settings import (DEFAULT_TONGYI_MODEL, DEFAULT_TIMEOUT_SECONDS
                      ,DEFAULT_MAX_RETRIES, API_KEY, BASE_URL)
...
class LLMClient:
    ...
    def __init__(self, 
        ...
        if not self.api_key:
            raise ValueError(
                "No Tongyi API key found. Please set environment variable "
        ...
        is_ok, err_msg = self.validate_api_access()
        if not is_ok:
            logger.error(f"API client initialization failed: {err_msg}")
    ...
    def validate_api_access(self) -> Tuple[bool, str]:
        ...
        """
        try:
            msg = [SystemMessage(content="you are a bot"), HumanMessage(content="Hello?")]
            self.client.invoke(msg)
            return True, ""
    ...
    def call_with_retry(self, 
                       prompt: str,
                       system_prompt: Optional[str] = None) -> Tuple[bool, str, str]:
        ...
        while retries <= self.max_retries:
            try:
                logger.info(f"API call attempt {retries + 1}/{self.max_retries + 1}")
                # Prepare messages
                messages = []
                ...
                response_text = self.client.invoke(messages).content
                duration = time.time() - start_time
                
                logger.info(f"API call successful in {duration:.1f}s")
    ...
    def analyze_single_finding(self, 
        ...
        try:
            ...
            if success:
                logger.info("Successfully parsed API response for single finding")
...
client = LLMClient()
# text = """Respond with EXACTLY this JSON structure (no markdown, no code blocks):
# {{
#   "original_severity": "HIGH",
#   "confidence_score": 8,
#   "keep_finding": true,
...
#   "justification": "Clear SQL injection vulnerability with specific exploit path"
# }}"""
# a,b,c = client.call_with_retry(text, "")
